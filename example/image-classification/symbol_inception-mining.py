...
import find_mxnet
import mxnet as mx
from mxnet.softmax_with_mining import SoftmaxWithMining
import sys
sys.path.insert(0, '../../example/numpy-ops/')
from numpy_softmax import NumpySoftmax
...
def get_symbol(num_classes = 10, force_mirroring=False):
    ...
    softmax_with_mining = SoftmaxWithMining(mining=True, sampling=True)
    softmax = softmax_with_mining(data=fc, name='softmax')
    #softmax_np = NumpySoftmax()
    #softmax = softmax_np(data=fc, name='softmax')
    return softmax
